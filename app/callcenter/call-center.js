...
//拨号数字面板配置
var dialPad=
	new top.Ext.Window({border:false,hideBorders:true,layout:'fit',title:'',width:170,resizeable:false,closeAction:'hide',items:[
...
function actionDialBox(obj) {
	var obj=top.Ext.getCmp("dialBox");
...
function showDialPad(x, y, title, callback, sendDtmf) {
	...
	top.Ext.getCmp("btnDialPadAction").setText(title);
...
function toolbarStatus() {
	//alert(ivr.MyState);
	var gIvrStat = ivr.MyState;
	console.log("state=" + gIvrStat);
	gIvrState = ivr.MyState;
	...
	//alert(gIvrState);
	if (gIvrStat <2) { //未连接或未注册
		if (gIvrStat <= -1) { //未连接
			ivrConnect();
		} else if (gIvrStat == 0 || gIvrStat == 1){ //已连接，未注册
			ivrLogin();
		}
		...
		bMonit.enable();
		bIntercept.enable();
	} else if (gIvrStat == 3 || gIvrStat == 5) { //摘机、暂停时不能使用
		pStatus.innerText=getPhoneAccount()+"\n（暂停）";
		bDial.disable();
		bAccept.disable();
		bTransfer.disable();
		...
		bMonit.disable();
		bIntercept.disable();
	} else if (gIvrStat == 6 || gIvrStat == 12 || gIvrStat == 13 || gIvrStat == 9 || gIvrStat == 10 || gIvrStat == 11) { //振铃、监听、强插等
		...
		bMonit.disable();
		bIntercept.disable();
	} else if (gIvrStat == 4 || gIvrStat == 7 || gIvrStat == 8) { //通话中、保持、外拨通话中
		pStatus.innerText=getRemoteURI()+"\n（"+getStrStatus()+"）";
		bDial.disable();
		bAccept.disable();
		bTransfer.enable();
...
function ivrConnect() {
	...
	console.log(getIvrServerAddress()+":"+getIvrServerPort());
